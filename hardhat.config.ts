// NOTE: hardhat-foundry is disabled to avoid remapping conflicts with hardhat-preprocessor
// The preprocessor handles Foundry remappings via the preprocess block below
...
// Safe getter for optional environment variables that may not be configured
function getOptionalVar(name: string): string | undefined {
  try {
    return vars.get(name);
  } catch {
    console.warn(`⚠️  Optional variable ${name} not configured`);
    return undefined;
  }
}

// Support both standard ecdsa eth accounts via mnemonic or private key for deployment
const mnemonic: string | undefined = getOptionalVar("MNEMONIC");
const deployerPrivateKey: string | undefined = getOptionalVar("DEPLOYER_PRIVATE_KEY");
...
const config: HardhatUserConfig = {
  ...
  paths: {
    artifacts: "./out", // have hardhat use ./out generated by Foundry
    cache: "./cache_hardhat", // Hardhat's own cache (NOT ./cache_forge)
    sources: "./contracts", // Shared source directory
    tests: "./test/hardhat",
  },
  solidity: {
    version: "0.8.20",
